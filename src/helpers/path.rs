...
pub fn list_clients(config_dir: &str) -> BearerResult<Vec<String>> {
    ...
    let mut ret = paths.map(
            |pth| pth.unwrap().path().file_name().unwrap().to_str().unwrap().to_string()
        ).filter(
            |pth| pth.ends_with(".toml")
        ).map(
            |pth| pth.as_str()[..pth.len() - 5].to_string()
        ).collect::<Vec<String>>();
    ret.sort();
    Ok(ret)
}
...
mod tests {
    ...
    fn test_list_clients_err() {
        ...
        assert_eq!(
            err,
            BearerError::ValueError("".to_string())
        )
    }
}
